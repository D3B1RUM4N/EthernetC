...
void ajouter_commutation(lan *l, switche sw, addMac mac){
    ...
    bool trouver = false;
    size_t num_port;
    appareil a = trouver_appareil_mac(l, sw.mac);
    sommet sa[degre(&l->g, a.id)];
    sommets_adjacents(&l->g, a.id, sa);
    //printf("\tdegres : %zu\n", degre(&l->g, a.id));
    for(size_t i = 0; i<degre(&l->g, a.id); i++){

        if((l->appareils[sa[i]].type == SWITCH && compare_mac(l->switches[l->appareils[sa[i]].index].mac, mac)) || (l->appareils[sa[i]].type == STATION && compare_mac(l->stations[l->appareils[sa[i]].index].mac, mac))){
            // le voisin est l'adrese voulue
            for(size_t j = 0; j<sw.nb_ports; j++){
                //printf("\t id de l'arete: %zu -- %zu || id de l'appareil cherché: %zu\n", sw.ports[j].arete.s1, sw.ports[j].arete.s2, trouver_appareil_mac(l, mac).id);
                if(sw.ports[j].arete.s1 == trouver_appareil_mac(l, mac).id || (sw.ports[j].arete.s2 == trouver_appareil_mac(l, mac).id)){
                    //on l'ajoute a la commutation
                    printf("\ttrouver voisin\n");
                    trouver = true;
                    num_port = j;
                    j = sw.nb_ports;
                    i = degre(&l->g, a.id);
        ...
        //a voir plus tard
        /*else if((l->appareils[sa[i]].type == STATION && sont_connectes(&l->g, sa[i], a.id))){
            // se port mene vers l'adresse voulue 
            for(size_t j = 0; j<sw.nb_ports; j++){
                printf("\t id de l'arete: %zu -- %zu || id de l'appareil cherché: %zu\n", sw.ports[j].arete.s1, sw.ports[j].arete.s2, trouver_appareil_mac(l, mac).id);

                if(sw.ports[j].arete.s1 == sa[i] || sw.ports[j].arete.s2 == sa[i]){
                    trouver = true;
                    num_port = j;
                    j = sw.nb_ports;
                    i = degre(&l->g, a.id);
    ...
    if(trouver){
        //printf("\tajout de la commutation\n");
        // on refait d'une autre maniere on remplace sw. en l->switches[l->appareils[a.index]].
        //on verifie l'alocation
        if(l->switches[a.index].nb_commutations == l->switches[a.index].commutations_capacite){
            l->switches[a.index].commutations_capacite *= 2;
            l->switches[a.index].commutations = realloc(l->switches[a.index].commutations, l->switches[a.index].commutations_capacite * sizeof(commutation));
        }
        l->switches[a.index].commutations[l->switches[a.index].nb_commutations].mac[0] = mac[0];
        l->switches[a.index].commutations[l->switches[a.index].nb_commutations].mac[1] = mac[1];
        l->switches[a.index].commutations[l->switches[a.index].nb_commutations].mac[2] = mac[2];
        l->switches[a.index].commutations[l->switches[a.index].nb_commutations].mac[3] = mac[3];
        l->switches[a.index].commutations[l->switches[a.index].nb_commutations].mac[4] = mac[4];
        l->switches[a.index].commutations[l->switches[a.index].nb_commutations].mac[5] = mac[5];

        l->switches[a.index].commutations[l->switches[a.index].nb_commutations].num_port = num_port;
        l->switches[a.index].nb_commutations++;
        ...
        sw.commutations[sw.nb_commutations].mac[0] = mac[0];
        sw.commutations[sw.nb_commutations].mac[1] = mac[1];
        sw.commutations[sw.nb_commutations].mac[2] = mac[2];
        sw.commutations[sw.nb_commutations].mac[3] = mac[3];
        sw.commutations[sw.nb_commutations].mac[4] = mac[4];
        sw.commutations[sw.nb_commutations].mac[5] = mac[5];
    
        sw.commutations[sw.nb_commutations].num_port = num_port;
        sw.nb_commutations ++;
        */

    }else{
        printf("\n\terreur: pas de port disponible\n");
    }

    if(sont_connectes(&l->g, a.id, trouver_appareil_mac(l, mac).id)){
